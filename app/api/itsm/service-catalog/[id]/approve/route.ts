...
// POST /api/itsm/service-catalog/[id]/approve - Approve a service
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const service = await serviceCatalogService.approveService(serviceId, parseInt(auth.userId));
    ...
    await serviceCatalogActivityService.createActivity({
      ...
      action: 'approved',
      description: `Approved service: ${service.name}`
    });
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: error.message || 'Failed to approve service' 
      },
      { status: 500 }
    );
  }
